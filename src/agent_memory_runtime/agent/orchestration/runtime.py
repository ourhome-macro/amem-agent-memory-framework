...
class AgentOrchestrator:
    ...
    async def _drive(self, progress: _ExecutionProgress, *, factory: _EventFactory,
                     token: CancellationToken) -> AsyncIterator[OrchestrationEvent]:
        from agent_memory_runtime.agent.orchestration.graph_driver import drive_dag
        async for event in drive_dag(self, progress, factory=factory, token=token):
            yield event
